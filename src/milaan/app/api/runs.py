...
router = APIRouter(prefix="/api/v1/runs", tags=["runs"], dependencies=[Depends(require_bearer_token)])
...
def create_run(
    ...
    manifest_hash = hashlib.sha256(
        "|".join(
            sorted([body.orders_file_id, body.gateway_settlement_file_id, body.bank_statement_file_id])
        ).encode()
    ).hexdigest()

    existing = db.execute(
        text("SELECT id, status FROM recon_run WHERE input_manifest_hash = :h AND ruleset_version = :rv"),
    ...
    if not ruleset_exists:
        if body.ruleset_version != "v1":
            raise HTTPException(status_code=422, detail=f"unknown ruleset_version {body.ruleset_version!r}")
        db.execute(
            text(
    ...
    db.execute(
        ...
        {
            "id": str(run_id), "ps": body.period_start, "pe": body.period_end,
            "rv": body.ruleset_version, "hash": manifest_hash, "llm_mode": settings.llm_mode.value,
        },
    )
    for source_type, upload in uploads.items():
        db.execute(
            ...
            {
                "id": upload["id"], "run_id": str(run_id), "source_type": source_type,
                "filename": upload["metadata"]["filename"],
                "sha256": upload["metadata"]["sha256"],
            },
        )
    append_entry(db, "operator", "run_created", "recon_run", run_id, {"idempotency_key": idempotency_key}, run_id=run_id)
    db.commit()
...
def list_runs(db: Session = Depends(get_db), limit: int = 50, offset: int = 0):
    ...
    return [
        {
            "id": str(r.id), "period_start": r.period_start.isoformat(), "period_end": r.period_end.isoformat(),
            "status": r.status, "record_count": r.record_count,
...
def get_run(run_id: str, db: Session = Depends(get_db)):
    ...
    return {
        "id": str(row.id), "period_start": row.period_start.isoformat(), "period_end": row.period_end.isoformat(),
        "status": row.status, "orchestrator_state": row.orchestrator_state,
        "ruleset_version": row.ruleset_version, "prompt_version": row.prompt_version,
        "rng_seed": row.rng_seed, "record_count": row.record_count, "llm_mode": row.llm_mode,
...
def get_run_metrics(run_id: str, db: Session = Depends(get_db)):
    row = db.execute(text("SELECT metrics FROM recon_run WHERE id = :id"), {"id": run_id}).fetchone()
    if not row:
        raise HTTPException(status_code=404, detail="run not found")
