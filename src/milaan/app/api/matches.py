...
def _group_to_dict(group_row, members) -> dict:
    return {
        ...
        "rule_id": group_row.rule_id,
        "ruleset_version": group_row.ruleset_version,
        "members": [
            {
                "entity_type": m.entity_type,
                "entity_id": str(m.entity_id),
                "allocated_amount": str(m.allocated_amount),
            }
            for m in members
        ],
    }
...
def list_matches(
    ...
    for g in groups:
        members = db.execute(
            text(
                "SELECT entity_type, entity_id, allocated_amount "
                "FROM match_member WHERE group_id = :gid"
            ),
            {"gid": str(g.id)},
        ).fetchall()
...
def get_match_group(group_id: str, db: Session = Depends(get_db)):
    g = db.execute(
        text(
            "SELECT id, tier, confidence, status, rule_id, ruleset_version "
            "FROM match_group WHERE id = :id"
        ),
        {"id": group_id},
    ).fetchone()
    if not g:
