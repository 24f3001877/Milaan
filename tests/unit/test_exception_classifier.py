...
def test_classifies_colliding_utr_as_duplicate_utr() -> None:
    ...
    bank_a = BankTxnEntity(
        id=uid(),
        value_date=D,
        narration="NEFT CR UTR-BAD",
        utr_extracted="UTR-BAD",
        credit=Money("200.00"),
        debit=Money("0.00"),  # deliberately not equal to line.net,
    )  # so T3's subset-sum can't accidentally
    bank_b = BankTxnEntity(  # recover this via a different route
        id=uid(),
        value_date=D,
        narration="NEFT CR UTR-BAD dup",
        utr_extracted="UTR-BAD",
        credit=Money("300.00"),
        debit=Money("0.00"),
    )
    ...
    records = classify_exceptions(
        [order], [line], [bank_a, bank_b], result, [], PERIOD_START, PERIOD_END
    )
...
def test_classifies_bank_with_colliding_utr_as_duplicate_utr() -> None:
    bank = BankTxnEntity(
        id=uid(),
        value_date=D,
        narration="NEFT CR UTR-BAD SETTLEMENT",
        utr_extracted="UTR-BAD",
        credit=Money("500.00"),
        debit=Money("0.00"),
    )
...
def test_classifies_fee_variance_from_t4_records() -> None:
    ...
    fv = FeeVarianceRecord(
        settlement_line_id=sid,
        expected_fee=Money("20.00"),
        expected_tax=Money("3.60"),
        reported_fee=Money("25.00"),
        reported_tax=Money("3.60"),
        delta=Money("5.00"),
        rate_card_version="v1",
        within_tolerance=False,
        instrument_resolved="upi",
    )
...
def test_fee_variance_within_tolerance_not_flagged() -> None:
    fv = FeeVarianceRecord(
        settlement_line_id=uid(),
        expected_fee=Money("20.00"),
        expected_tax=Money("3.60"),
        reported_fee=Money("20.00"),
        reported_tax=Money("3.60"),
        delta=Money("0.00"),
        rate_card_version="v1",
...
def test_many_orders_one_credit_is_not_flagged_as_partial_settlement() -> None:
    ...
    bank = BankTxnEntity(
        id=uid(),
        value_date=D,
        narration="NEFT CR UTR-DAY SETTLEMENT",
